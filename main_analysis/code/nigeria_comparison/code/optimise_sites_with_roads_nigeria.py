"""
NIGERIA COMPARISON: ROAD-CONSTRAINED SURVEILLANCE OPTIMISATION
---------------------------------------------------------------

Purpose
-------
Runs surveillance site optimisation for Nigerian state landscapes under
road-accessibility constraints.

Candidate surveillance locations are restricted to host grid cells within a
specified distance of the road network. Simulated annealing is used to choose
NSITES surveillance locations that maximise the probability of detecting
infection before a prevalence threshold is reached.
...
Inputs
------
- Host distribution (.joblib) in:
    nigeria_comparison/host_distributions/

- Road network shapefile in:
    nigeria_comparison/road_networks/

- Simulation outputs generated by:
    run_sims_nigeria.py

Command-line arguments
----------------------
STATE_ID       : state identifier (e.g. "ABIA", "BENUE")
SURVEY_FREQ    : survey frequency (integer)
NSITES         : number of surveillance sites (integer)

Example:
    python optimise_sites_with_roads_nigeria.py ABIA 104 10
...
Notes
-----
Uses checkpointing functionality to allow long simulated annealing runs
...
#ROAD_JA = sys.argv[1]
AREA_JA = sys.argv[1]
SURVEY_FREQ_JA = int(sys.argv[2])  
NSITES_JA=int(sys.argv[3]) 
...
INPUT_ROADS = BASE_DIR / "road_networks"

# Simulation outputs (produced by run_sims_nigeria.py)
...
filename_road = f"roads_{AREA_JA}.shp"
filename_save = f"area{AREA_JA}_surveyfreq{SURVEY_FREQ_JA}_nsites{NSITES_JA}"

# Checkpoint file for continuing after Slurm timeout
...
### Obtain road
shp_road = gpd.read_file(INPUT_ROADS / filename_road)
...
### Optimisation (SA) ===========================================================================================================
# n_iter =50000, lastN=10000, checkpoint_every=500
n_iter =50000 # 
...
road_length, road_coverage_area, RCAP,road_coverage_plant,RMS_toRoad_wtd,RMS_toRoad, RMS_toAllowed_wtd, RMS_toAllowed, RMS_toOpt_wtd, RMS_toOpt,num_red_optimal, num_red_allowed, prop_red_allowedred,prop_red_allowed = opt_metrics(shp_road, host_positions, host_population, best_config)
...
## plt.legend()
plt.savefig(TRACE_DIR / f"plot_OptTrace_{filename_save}.pdf", dpi=600)
...
plt.legend()
plt.savefig(MAP_DIR / f"plot_OptResult_{filename_save}.pdf", dpi=600)
plt.clf # clear figure
plt.close(fig)
