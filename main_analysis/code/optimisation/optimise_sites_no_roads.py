"""
MAIN ANALYSIS: OPTIMISE SURVEILLANCE SITES WITHOUT ROAD CONSTRAINTS
----------------------------------------------------------------

Purpose
-------
Computes an idealised baseline optimisation in which surveillance sites may be
placed anywhere on the host landscape (i.e., no accessibility constraints).

This represents a theoretical upper bound on surveillance performance and is
used for comparison with road-constrained optimisation scenarios.

Inputs
------
- Epidemic simulation outputs:
    main_analysis/Outputs/simulations/df_simsXX.joblib

Command-line arguments
----------------------
AREA_JA        : identifier for study area (e.g., 01, 02, ...)
SURVEY_FREQ_JA : survey frequency
NSITES_JA      : number of surveillance sites

Outputs
-------
Writes optimisation results to:
    main_analysis/Outputs/optimisation/

Notes
-----
Unlike the road-constrained optimisation, this script allows all host grid
cells to be considered as candidate surveillance locations.
"""
...
filename_save = f'NoRoad_area{AREA_JA}_surveyfreq{SURVEY_FREQ_JA}_nsites{NSITES_JA}'
...
best_config, config_list, temp_list, objVal_list, n_iter_actual = simulated_annealing(data_sim=sim_data, 
                                                                        ...
                                                                        lastN=10000,earlystop_tol=0.0001) #smaller tolerance



road_length, road_coverage_area, road_coverage_plant,RMS_toRoad_wtd,RMS_toRoad, RMS_toAllowed_wtd, RMS_toAllowed, RMS_toOpt_wtd, RMS_toOpt,num_red_optimal, num_red_allowed, prop_red_allowedred,prop_red_allowed = opt_metrics(None, host_positions, host_population, best_config)
