"""
MAIN ANALYSIS: OPTIMISE SURVEILLANCE SITES WITH ROAD CONSTRAINTS
--------------------------------------------------------------

Purpose
-------
Optimises surveillance site locations under accessibility constraints imposed
by road networks.

Candidate surveillance locations are restricted to host grid cells within a
specified distance (1 km) of the provided road network. A simulated annealing
algorithm is then used to select NSITES locations that maximise the probability
of detecting infection before a prevalence threshold is reached.
...
- Road network shapefiles:
    main_analysis/road_patterns/roadnetworkXX.shp

Command-line arguments
----------------------
ROAD_JA        : identifier for road scenario (e.g., 01, 02, ...)
AREA_JA        : identifier for study area (e.g., 01, 02, ...)
SURVEY_FREQ_JA : survey frequency
NSITES_JA      : number of surveillance sites

Example:
    python optimise_sites_with_roads.py 01 01 104 10
...
Including:
- objective value traces
- optimal site configurations
- performance metrics
- visualisations of optimised sites

Notes
-----
This script represents the primary optimisation workflow used in the manuscript.
"""
...
ROAD_JA = sys.argv[1]
AREA_JA = sys.argv[2]
SURVEY_FREQ_JA = int(sys.argv[3])  
NSITES_JA=int(sys.argv[4]) 
