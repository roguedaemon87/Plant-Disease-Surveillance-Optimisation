"""
OUTLIER CASE ANALYSIS: OPTIMISE SURVEILLANCE SITES WITH ROAD CONSTRAINTS
----------------------------------------------------------------------

Purpose
-------
Runs road-constrained surveillance optimisation for diagnostic outlier cases.

Candidate surveillance locations are restricted to host grid cells within a
specified distance (1 km) of a given road network shapefile. Simulated
annealing is used to choose NSITES surveillance locations that maximise the
probability of detecting infection before a prevalence threshold is reached.

This script is intended for investigating specific outlier configurations
(e.g. road networks that are far from hotspots), not for reproducing the main
manuscript optimisation runs.
...
- Road network shapefiles:
    main_analysis/code/optimisation/outlier_cases/inputs/road_patterns/roadnetworkYY.shp

Command-line arguments
----------------------
ROAD_JA        : road pattern identifier (e.g. "01_0", "01_1", "05_2")
AREA_JA        : area identifier (e.g. "01", "05")
SURVEY_FREQ_JA : survey frequency (integer)
NSITES_JA      : number of surveillance sites (integer)

Example:
    python optimise_sites_with_roads_outlier_cases.py 01_0 01 104 10
...
including:
- optimisation traces (objective values)
...
Notes
-----
This script differs from the main analysis optimisation only in that it is run
on a subset of inputs chosen as diagnostic outlier cases.
"""
...
filename_save = f'road{ROAD}_area{AREA}_surveyfreq{SURVEY_FREQ}_nsites{NSITES}'
