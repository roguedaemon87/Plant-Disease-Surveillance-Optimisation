"""
OUTLIER CASE ANALYSIS: OPTIMISE SURVEILLANCE SITES WITHOUT ROAD CONSTRAINTS
-------------------------------------------------------------------------

Purpose
-------
Computes an idealised baseline optimisation for the outlier-case host
landscapes, where surveillance sites can be placed anywhere on the host grid
(i.e., no accessibility constraints due to roads).

This provides an upper-bound benchmark for detection performance, to compare
against the corresponding road-constrained outlier-case optimisation runs.

Inputs
------
- Simulation outputs (generated by run_sims_outlier_cases.py):
    main_analysis/Outputs/outlier_cases/simulations/df_simsXX.joblib

Command-line arguments
----------------------
AREA_JA        : area identifier (e.g. "01", "05")
SURVEY_FREQ_JA : survey frequency (integer)
NSITES_JA      : number of surveillance sites (integer)

Example:
    python optimise_sites_no_roads_outlier_cases.py 01 104 10

Outputs
-------
Writes outputs to:
    main_analysis/Outputs/outlier_cases/optimisation/

Notes
-----
This script is part of the diagnostic outlier-case investigation and is not
...
from optimisation import site_loc_allowed, logistic_forOpt, objective_more_optimised, simulated_annealing, opt_metrics
...
filename_save = f'NoRoad_area{AREA}_surveyfreq{SURVEY_FREQ}_nsites{NSITES}'
...
sim_data = load(os.path.join(dir_home,subdir_sim,filename_sim))  # CHECK HERE
...
site_loc_allowed_ID, cells_near_road_ID = site_loc_allowed(roadnetwork=None, host_pos=host_positions, host_pop=host_population, accessible_dist=1000)  # 1km accessibility # CHECK HERE
...
ax.scatter(host_positions[site_loc_allowed_ID, 0], host_positions[site_loc_allowed_ID, 1], marker='s', s=30, facecolors='none', edgecolors='black', label='sites allowed for survey', alpha=0.6)  # CHECK HERE
